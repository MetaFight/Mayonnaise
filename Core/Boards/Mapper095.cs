...
namespace MyNes.Core
{
    ...
    class Mapper095 : Board
    {
        ...
        public override byte ReadNMT(ref int address)
        {
            if (flag_c)
            {
                return nmt_banks[(chr_reg[((address >> 10) & 3) + 2] & 0x20) >> 5][address & 0x3FF];
            }
            else
            {
                switch ((address >> 10) & 3)
                {
                    case 0:
                    case 1: return nmt_banks[(chr_reg[0] & 0x20) >> 5][address & 0x3FF];
                    case 2:
                    case 3: return nmt_banks[(chr_reg[1] & 0x20) >> 5][address & 0x3FF];
                }
            }
        ...
        public override void WriteNMT(ref int address, ref byte data)
        {
            if (flag_c)
            {
                nmt_banks[(chr_reg[((address >> 10) & 3) + 2] & 0x20) >> 5][address & 0x3FF] = data;
            }
            else
            {
                switch ((address >> 10) & 3)
                {
                    case 0:
                    case 1: nmt_banks[(chr_reg[0] & 0x20) >> 5][address & 0x3FF] = data; break;
                    case 2:
                    case 3: nmt_banks[(chr_reg[1] & 0x20) >> 5][address & 0x3FF] = data; break;
                }
            }
        }
