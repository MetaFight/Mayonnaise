...
namespace MyNes.Core
{
    ...
    class Mapper045 : Board
    {
        ...
        public override void HardReset()
        {
            ...
            flag_c = flag_p = false;
            address_8001 = 0;
            prg_and = 0x3F; prg_or = 0x00; chr_and = 0xFFF; chr_or = 0x000;
            prg_reg = new int[4];
            prg_reg[0] = 0;
            prg_reg[1] = 1;
        ...
        public override void WriteSRM(ref int address, ref byte data)
        {
            ...
            else
            {
                switch (regCounter)
                {
                    case 0: chr_or = (chr_or & 0xFF00) | data; SetupCHR(); break;
                    case 1: prg_or = data; SetupPRG(); break;
                    case 2:
                        {
                            // chr_and = 0xFF >> ~S_bits;
                            //chr_and = 0xFF >> ~(data & 0xF);
                            if ((data & 0x8) == 0x8)
                                chr_and = ((1 << ((data & 0x7) + 1)) - 1);
                            else
                                chr_and = ((data > 0) ? 0 : ~0);
                            chr_or = (chr_or & 0x00FF) | ((data & 0xF0) << 4);
                            SetupCHR();
                            break;
                        }
                    case 3:
                        {
                            locked = (data & 0x40) == 0x40;
